...
.venv:
	if [ ! -e ".venv/bin/activate_this.py" ] ; then virtualenv -p python3 --clear .venv ; fi
...
deps: .venv
	. .venv/bin/activate && pip3 install -U -r requirements.txt -t ./src/libs

dev_deps: .venv
	. .venv/bin/activate && pip3 install -U -r dev_requirements.txt
